...
[CustomEditor(typeof(CPC_Path))]
public class CPC_PathInspector : Editor
{
    private CPC_Path t;
    ...
    private GUIContent relativeButtonContent = new GUIContent("Relative", "Relative position with Camera's position");
    private GUIContent worldPointButtonContent = new GUIContent("WorldPoint", "WorldPoint");

    //Serialized Properties
    private SerializedObject serializedObjectTarget;
    private SerializedProperty useMainCameraProperty;
    private SerializedProperty useRelativePositionProperty;
    ...
    private SerializedProperty visualFrustumRangeProperty;
    ...
    void GetVariableProperties()
    {
        ...
        useRelativePositionProperty = serializedObjectTarget.FindProperty("useRelativePosition");
        ...
        visualFrustumRangeProperty = serializedObjectTarget.FindProperty("visual.MaxRange");
    ...
    void SetupReorderableList()
    {
        ...
        const float spacingYHorizontal = 2;
        const float spacingXVerical = 2;
        


        const float WidthlabelChained = 40;
        const float LEFT_PANEL = WidthlabelChained;

        const float WidthRemoveButton = 20;
        const float WidthDelayTime = 20;
        const float Width_Time_PerSeg = 20;

        const float RIGHT_PANEL = WidthDelayTime + spacingXVerical + WidthRemoveButton + Width_Time_PerSeg;


        const float WIDTH_LEFTandRIGHT_PANEL = LEFT_PANEL + spacingXVerical + RIGHT_PANEL;

        const int AmountElementAtMid = 3;
        const int AmountElementInHorizontalMidLayout = 2;
        ...
        pointReorderableList.drawElementCallback = (rect, index, active, focused) =>
        {
            ...
            float RECT_WIDTH = rect.width;

            Vector2 RIGHT_POSITION = new Vector2(rect.width - RIGHT_PANEL, startRectY);

            rect.height -= 2;
            float FULL_WIDTH = rect.width - 16 * (hasScrollBar ? 1 : 0);
            float WidthMidElement = (FULL_WIDTH - WIDTH_LEFTandRIGHT_PANEL) / AmountElementAtMid - 1;


            #region ========= Left ============
            #region Chained
            rect.width = WidthlabelChained;
            rect.height /= 2;
            ...
            rect.y += pointReorderableList.elementHeight / AmountElementInHorizontalMidLayout - 4;
            //rect.x += rect.width + 2;
            EditorGUI.BeginChangeCheck();
            ...
            rect.y = startRectY;
            rect.height *= 2;
            rect.x += rect.width + spacingYHorizontal;
            rect.width = (FULL_WIDTH - WIDTH_LEFTandRIGHT_PANEL) / AmountElementAtMid;
            rect.height = rect.height / AmountElementInHorizontalMidLayout - 1;
            ...
            rect.y += rect.height + spacingYHorizontal;
            ...
            rect.x = RECT_WIDTH - Width_Time_PerSeg;
            rect.width = 20;

            // EditorGUI.LabelField(rect, "delay(s)");
            //rect.y += rect.height + spacingYHorizontal;
            EditorGUI.BeginChangeCheck();
            timePerSeg = EditorGUI.FloatField(rect, t.points[index].timePerSeg);
            if (EditorGUI.EndChangeCheck())
            {
                t.points[index].timePerSeg = timePerSeg;
            }
            ...
            rect.x = RECT_WIDTH + spacingXVerical;
            rect.width = 20;

            // EditorGUI.LabelField(rect, "delay(s)");
            //rect.y += rect.height + spacingYHorizontal;
            EditorGUI.BeginChangeCheck();
            delayTime = EditorGUI.FloatField(rect, t.points[index].delayToNextPoint);
            if (EditorGUI.EndChangeCheck())
            {
                t.points[index].delayToNextPoint = delayTime;
            }
            #endregion

            #region Button Remove "X"
            rect.height = RECT_HEIGHT;
            rect.y = startRectY;

            rect.x = RECT_WIDTH + spacingXVerical + WidthRemoveButton;
            rect.width = 20;
        ...
        pointReorderableList.drawHeaderCallback = rect =>
        {
            ...
            rect.width = (fullWidth - 78) / AmountElementAtMid;
    ...
    protected bool oldRelativeSetting = false;
    void DrawBasicSettings()
    {
        ...
        if (!t.IsUseRelative())
        {
            ...
            if (GUILayout.Button(relativeButtonContent))
            {
                t.ToggleRelative(true);
            }
        }
        ...
        {
            if (GUILayout.Button(worldPointButtonContent))
            {
                t.ToggleRelative(false);
            }
        }
        ...
        selectedCameraProperty.objectReferenceValue = (Transform)EditorGUILayout.ObjectField(selectedCameraProperty.objectReferenceValue, typeof(Transform), true);
        GUI.enabled = true;
        //oldRelativeSetting = useRelativePositionProperty.boolValue;
        //useRelativePositionProperty.boolValue = GUILayout.Toggle(useRelativePositionProperty.boolValue, "Set to relative position", GUILayout.Width(Screen.width / 3f));
        //if (useRelativePositionProperty.boolValue != oldRelativeSetting)
        //{
        //    oldRelativeSetting = useRelativePositionProperty.boolValue;
        //    t.SetUpPath();
        //}
    ...
    void DrawVisualDropdown()
    {
        ...
        if (visualFoldout)
        {
            ...
            visualFrustumRangeProperty.floatValue = EditorGUILayout.FloatField("Range Frustum", visualFrustumRangeProperty.floatValue);
        ...
        if (GUILayout.Button(addPointContent))
        {
            ...
            switch (waypointMode)
            {
                ...
                case CPC_ENewWaypointMode.WorldCenter:
                    t.points.Add(new CPC_Point(t.transform.position, Quaternion.identity));
                    break;
                default:
